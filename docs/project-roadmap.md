...
- **Countdown timer** (Pro): Per-bar countdown to fixed instant (site-TZ absolute epoch) or evergreen per-visitor duration (localStorage-seeded). Three UI styles (boxes/flip/circular); flip/circular degrade to static under prefers-reduced-motion. Renders as dedicated block between text and button; hides at zero. Lite: UI disabled with Go-Pro notice; ticker JS stripped from build.
