...
if ( ! class_exists( 'FBNotificationCross' ) ) {
	class FBNotificationCross {
			...
			add_action(
				...
				function () {
					if ( ! $this->is_plugin_exist() ) {
					 	$notificationOption = get_option( "yay_notification_{$this->pluginPrefix}_cross" ); //Save the next time notification will appear
					if ( false === $notificationOption || time() >= $notificationOption ) {
						add_action( 'admin_notices', array( $this, 'add_notification' ) );
					 	add_action( "wp_ajax_yay_notification_{$this->pluginPrefix}_cross", array( $this, 'ajax_set_notification' ) );
					}

					add_action( 'admin_footer', array( $this, 'add_global_script_styles' ) );
					
					add_action( "wp_ajax_yay_notification_{$this->pluginPrefix}_cross_install", array( $this, 'ajax_install_plugin' ) );
					add_action( "wp_ajax_yay_notification_{$this->pluginPrefix}_cross_hide", array( $this, 'ajax_hide_cross' ) );
					}
				}
			);
		}
		...
		public function need_update_option() {
			$time = 5 * 60;
			update_option( "yay_notification_{$this->pluginPrefix}_cross", $time );
		}
		...
		public function add_global_script_styles() {
			...
			if ( function_exists( 'get_current_screen' ) ) {
				...
				if ( ! in_array( $screen->id, array_merge( array( 'plugins' ) ) ) ) {
					return;
				}
			} else {
				return;
			...
			wp_register_script( "yay-notification-{$this->pluginPrefix}-cross", $this->pluginDirURL . 'assets/js/cross.js', array( 'jquery' ), '1.0', true );
			wp_localize_script(
				"yay-notification-{$this->pluginPrefix}-cross",
				'yayNotificationCross',
				array(
			...
			wp_enqueue_script( "yay-notification-{$this->pluginPrefix}-cross" );
			?>
			<style>
		...
		public function add_notification() {
			...
			?>
			<div class="notice notice-info is-dismissible" id="yay-ads-wrapper">
				<div class="yay-d-row yay-justify-between">
					<div class="yay-ads-info">
					<h4 class="yay-ads-title"><?php _e( 'Recommend', 'ninjateam-whatsapp' ); ?></h4>
					<p>To easily manage your files in WordPress media library with folders, please try FileBird plugin.</p>
					<div class="yay-btn-row">
						<a class="button button-primary fbv-cross-install yay-ads-install-button" rel="noopener noreferrer" href="javascript:;">
					...
					<img class="yay-ads-img" src="<?php echo $this->pluginDirURL . 'assets/img/FB_Wireframe.png'; ?>" alt="filebird">
				</div>
			</div>
			<style>
				.yay-d-row{-webkit-box-align:center;-ms-flex-align:center;align-items:center;display:-webkit-box;display:-ms-flexbox;display:flex}.yay-justify-between{-webkit-box-pack:justify;-ms-flex-pack:justify;justify-content:space-between}.yay-ads-info{padding:15px 0 30px}@media screen and (max-width:782px){.yay-ads-info{padding:0}}.yay-ads-info p{margin-bottom:25px}.yay-ads-title{font-size:16px;margin:0 0 15px}.yay-ads-img{max-width:252px}.yay-btn-row{display: flex;align-items: center;}.yay-btn-row>.button{margin-right:10px}
			</style>
			<?php
		}
		...
		public function ajax_set_notification() {
			...
			//Save after 30 days
			update_option( "yay_notification_{$this->pluginPrefix}_cross", time() + ( 30 * 60 * 60 * 24 ) );
			wp_send_json_success();
		}
...
FBNotificationCross::get_instance();
